/**
 * Menu sisi (sidebar) halaman pengelolaan.
 *
 * Tiap sub-fitur berdiri sendiri sebagai satu halaman input, dikelompokkan
 * supaya pengurus desa gampang menemukan yang dicari — bukan satu form raksasa
 * berisi segalanya. Menambah sub-fitur baru cukup menambah satu butir di sini
 * dan membuat halaman route-nya.
 *
 * `ikon` mengacu ke satu jalur SVG di IKON di bawah. Path memakai gaya
 * Heroicons (garis, stroke) supaya seragam dan ringan — tanpa pustaka ikon.
 */

export type ButirMenuAdmin = {
  label: string;
  href: string;
  ikon: keyof typeof IKON;
  /** true kalau halamannya sudah bisa diisi; false = masih disiapkan. */
  siap: boolean;
};

export type KelompokMenuAdmin = {
  judul?: string;
  butir: ButirMenuAdmin[];
};

export const MENU_ADMIN: KelompokMenuAdmin[] = [
  {
    butir: [{ label: "Dashboard", href: "/admin", ikon: "rumah", siap: true }],
  },
  {
    judul: "Profil Desa",
    butir: [
      { label: "Visi & Misi", href: "/admin/visi-misi", ikon: "target", siap: true },
      { label: "Sejarah", href: "/admin/sejarah", ikon: "buku", siap: true },
  ...
  },
  {
    ...
    butir: [
      ...
      { label: "Agenda", href: "/admin/agenda", ikon: "kalender", siap: true },
      { label: "Galeri", href: "/admin/galeri", ikon: "foto", siap: true },
    ],
  },
  {
    judul: "Layanan",
    butir: [
      { label: "Kontak & Layanan", href: "/admin/kontak", ikon: "telepon", siap: true },
    ],
  },
];

/** Jalur ikon SVG (viewBox 0 0 24 24, stroke). Satu path per ikon. */
export const IKON = {
  rumah:
